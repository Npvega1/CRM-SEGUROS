-- =====================================================
-- MIGRACIÓN: 00001_clients_policies.sql
-- CRM Multi-tenant para Agencias de Seguros
-- Módulo 01: Gestión de Clientes y Pólizas
-- =====================================================
...
-- Línea de seguro
CREATE TYPE policy_line AS ENUM (
  'vida',
  'auto',
  'salud',
  'hogar',
...
CREATE TABLE clients (
  ...
  -- Constraint: documento único por tenant
...
-- Índices pg_trgm para búsqueda full-text fuzzy
CREATE INDEX idx_clients_full_name_trgm ON clients USING GIN(full_name gin_trgm_ops);
CREATE INDEX idx_clients_doc_number_trgm ON clients USING GIN(doc_number gin_trgm_ops);
...
CREATE TABLE policies (
  ...
  -- Constraint: número de póliza único por tenant
...
-- Historial de cambios de estado de pólizas
-- =====================================================
...
-- Índices para policy_history
CREATE INDEX idx_policy_history_policy_id ON policy_history(policy_id);
CREATE INDEX idx_policy_history_changed_at ON policy_history(changed_at);
...
CREATE INDEX idx_automation_queue_tenant_id ON automation_queue(tenant_id);

-- =====================================================
-- TABLA: automation_logs (stub para M06)
-- Logs de automatizaciones ejecutadas
-- =====================================================
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    -- Si la póliza se activa, crear entrada en automation_queue para M05 (facturación)
...
CREATE POLICY "Ver clientes del tenant"
...
-- Crear clientes en el tenant
CREATE POLICY "Crear clientes en el tenant"
  ON clients FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = auth.tenant_id()
    AND auth.user_role() IN ('admin', 'senior_agent', 'agent')
  );

-- Actualizar clientes del tenant
CREATE POLICY "Actualizar clientes del tenant"
...
CREATE POLICY "Eliminar clientes del tenant"
...
-- Ver pólizas del tenant
CREATE POLICY "Ver pólizas del tenant"
...
-- Crear pólizas en el tenant
CREATE POLICY "Crear pólizas en el tenant"
...
-- Actualizar pólizas del tenant
CREATE POLICY "Actualizar pólizas del tenant"
...
-- Eliminar pólizas (solo admin)
CREATE POLICY "Eliminar pólizas del tenant"
...
-- Ver historial de pólizas del tenant
CREATE POLICY "Ver historial de pólizas del tenant"
  ...
  USING (
    EXISTS (
      ...
      AND (p.tenant_id = auth.tenant_id() OR auth.is_superadmin())
    )
  );

-- Insertar historial (solo vía trigger, pero permitir para el tenant)
CREATE POLICY "Insertar historial de pólizas"
...
CREATE POLICY "Ver automation_queue del tenant"
  ON automation_queue FOR SELECT
  TO authenticated
  USING (tenant_id = auth.tenant_id() OR auth.is_superadmin());

CREATE POLICY "Insertar automation_queue"
...
CREATE POLICY "Ver automation_logs del tenant"
  ON automation_logs FOR SELECT
  TO authenticated
  USING (tenant_id = auth.tenant_id() OR auth.is_superadmin());

CREATE POLICY "Insertar automation_logs"
  ON automation_logs FOR INSERT
  TO authenticated
  WITH CHECK (tenant_id = auth.tenant_id() OR auth.is_superadmin());
...
BEGIN
  ...
  SELECT 
    ...
    GREATEST(
      ...
      similarity(c.doc_number, p_query)
    ) AS similarity_score
  FROM clients c
...
-- =====================================================
-- STORAGE BUCKET para documentos de pólizas
-- =====================================================
-- NOTA: Ejecutar esto en el Dashboard de Supabase > Storage
-- O usar la API de admin para crear el bucket
...
-- TO authenticated
-- WITH CHECK (bucket_id = 'policy-documents' AND (storage.foldername(name))[1] = auth.tenant_id()::text);

-- CREATE POLICY "Usuarios pueden ver documentos de su tenant"
-- ON storage.objects FOR SELECT
-- TO authenticated
-- USING (bucket_id = 'policy-documents' AND (storage.foldername(name))[1] = auth.tenant_id()::text);
...
COMMENT ON TABLE automation_queue IS 'Cola de automatizaciones pendientes (M06)';
COMMENT ON TABLE automation_logs IS 'Logs de automatizaciones ejecutadas (M06)';

COMMENT ON FUNCTION validate_policy_status_transition() IS 'Valida transiciones válidas de estado de póliza';
COMMENT ON FUNCTION record_policy_status_change() IS 'Registra cambios de estado en policy_history';
COMMENT ON FUNCTION search_clients(UUID, TEXT, INT) IS 'Búsqueda fuzzy de clientes con pg_trgm';
COMMENT ON FUNCTION get_expiring_policies(UUID, INT[]) IS 'Obtiene pólizas que vencen en los días especificados';
